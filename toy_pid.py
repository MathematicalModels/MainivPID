...
def test_pid(P=0.2, I=0.0, D=0.0, L=200):
    ...
    signal_record = []  #record sigal = \Sum (outputs)
    signal_with_quit_record = [] #record signal + quitvalue
    ...
    for i in range(0, END):
        ...
        pid.SetPoint = 3*np.sin(Env4_x[i]/100) + 5 + np.random.normal(0, 0.1, 1)[0]
        ...
        signal_record.append(pid.signal)
        ...
        if output_pattern_record[i] > threshold:
            ...
            if output_pattern_record[i-1] <= threshold:
                pid.signal_with_quit = pid.signal_with_quit - 10
        if i > 2 and output_pattern_record[i-1] > threshold and output_pattern_record[i] < threshold:
                pid.signal_with_quit =pid.signal_with_quit + 10
        signal_with_quit_record.append(pid.signal_with_quit)
    ...
    plt.subplot(222) #plot the signal vs the setpoint
    plt.plot(time_list, setpoint_record)
    plt.plot(time_list, signal_record)
    plt.xlim((0, L))
    plt.ylim(-10, 20)
    plt.xlabel('time (s)')
    plt.ylabel('Values')
    plt.title('Signal (Yellow) vs Set Point')
    plt.grid(True)

    plt.subplot(223) #plot the output vs the setpoint
    plt.plot(time_list, setpoint_record)
    plt.plot(time_list, signal_with_quit_record)
    plt.xlim((0, L))
    plt.ylim(-20, 20)
    plt.xlabel('time (s)')
    plt.ylabel('Values')
    plt.title('Action Value  (Yellow) vs Set Point')
    plt.grid(True)

    plt.subplot(224) #plot
